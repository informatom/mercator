class OrdersController < OpensteamController
  before_filter :find_empty_category
  before_filter :require_customer
  before_filter :find_active_order, :only => [ :edit, :update, :destroy, :place, :finish, :mpay24, :payment, :payment_success, :payment_error ]
  before_filter :check_order_attributes, :only => [:create ]

  helper :ivellio_order

  def place
    if @order.payment_state != "BILLED" && @order.payment_state != "ERROR" && @order.online_banking?
      redirect_to :action => :payment
      return
    end

    if @order.payment_state == "ERROR" && @order.online_banking?
      redirect_to :action => :payment
      return
    end
  ...
  def payment
    begin
      ...
      cart_xml = @cart.to_mpay24_xml( @order.order_number, @order.delivery?, current_customer )
...
#      @wmbi_url = "https://test.mPAY24.com/app/bin/checkout/payment/f5c4fac03f524881a0b5fc1f8ff03063\n"
  ...
  def check_order_attributes
    returning( params[:order] ) do |params_order|
      params_order[:shipping_name]        = params_order[:billing_name] if params_order[:shipping_name].blank?
      params_order[:shipping_street]      = params_order[:billing_street] if params_order[:shipping_street].blank?
      params_order[:shipping_postal]      = params_order[:billing_postal] if params_order[:shipping_postal].blank?
      params_order[:shipping_city]        = params_order[:billing_city] if params_order[:shipping_city].blank?
      params_order[:shipping_state_code]  = params_order[:billing_state_code] if params_order[:shipping_state_code].blank?
    end
  end

  def check_cart
    if @cart.empty?
      flash[:error] = t( "checkout_cancelled_empty_cart")
